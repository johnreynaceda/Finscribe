...
class DescriptiveAnalytic extends Component
{
    ...
    public function mount(){
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $income = Income::whereMonth('date', $i - 1)->whereYear('date', now()->year)->sum('total_sales');
            $expense = Expense::whereMonth('date', $i - 1)->whereYear('date', now()->year)->sum('total_expense');
